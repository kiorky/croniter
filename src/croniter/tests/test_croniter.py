...
class CroniterTest(base.TestCase):
    ...
    def _test_cron_ranges(
        self, res_generator, expr, wanted, iterations=None, start_time=None
    ):
        rets = res_generator(expr, iterations=iterations, start_time=start_time)
        for ret in rets:
            self.assertEqual(wanted, ret)

    def _test_mth_cron_ranges(
        self, expr, wanted, iterations=None, res_generator=None, start_time=None
    ):
