...
class CroniterSpeedTest(base.TestCase):
    ...
    def test_not_long_time(self):
        iterations = int(os.environ.get("CRONITER_TEST_SPEED_ITERATIONS", "500"))
        globs = globals()
        globs.update(locals())
        t = Timer("self.run_long_test(iterations)", globals=globs)
        ret = t.timeit(5)
        self.assertTrue(ret < 5, "Regression in croniter speed detected")
